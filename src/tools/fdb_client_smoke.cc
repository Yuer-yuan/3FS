 ...
 public:
  ...
  fdb_error_t start() {
    ...
    thread_ = std::thread([this] {
      networkError_.store(fdb_run_network(), std::memory_order_release);
    });
    return 0;
  }
...
fdb_error_t configureTransaction(FDBTransaction *transaction) {
  fdb_error_t error = fdb_transaction_set_option(
      transaction, FDB_TR_OPTION_ACCESS_SYSTEM_KEYS, nullptr, 0);
  if (error != 0) {
    return error;
  }
  return fdb_transaction_set_option(
      transaction, FDB_TR_OPTION_TIMEOUT,
...
Result runTransaction(const Arguments &arguments) {
  ...
  fdb_transaction_set(
      writeTransaction.get(), reinterpret_cast<const std::uint8_t *>(key.data()),
  ...
  Future read(fdb_transaction_get(
      readTransaction.get(), reinterpret_cast<const std::uint8_t *>(key.data()),
  ...
  result.valueMatch =
      present && valueLength == static_cast<int>(arguments.value.size()) &&
      std::string_view(reinterpret_cast<const char *>(value), valueLength) ==
          arguments.value;
...
void printResult(const Result &result) {
  const char *message =
      result.error == 0
          ? "success"
          : (result.error > 0 ? fdb_get_error(result.error)
                              : "internal smoke error");
  bool passed = result.setOk && result.getOk && result.valueMatch &&
                result.error == 0;
  std::cout << "{\"api_version\":" << FDB_API_VERSION
            << ",\"set_ok\":" << (result.setOk ? "true" : "false")
            << ",\"get_ok\":" << (result.getOk ? "true" : "false")
            << ",\"value_match\":"
            << (result.valueMatch ? "true" : "false")
...
bool parseArguments(int argc, char **argv, Arguments &arguments) {
  ...
  return clusterSeen && keySeen && valueSeen && !arguments.clusterFile.empty() &&
         !arguments.key.empty();
}
...
int main(int argc, char **argv) {
  ...
  printResult(result);
  return result.setOk && result.getOk && result.valueMatch && result.error == 0
             ? EXIT_SUCCESS
             : EXIT_FAILURE;
}
